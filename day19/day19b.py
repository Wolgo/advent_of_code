...
def search(ore_cost, clay_cost, obs_ore_cost, obs_clay_cost, geo_ore_cost, geo_obs_cost, time_left, current_ore,
    ...
    if best_result >= current_geo + time_left * robot_geo + (time_left * 5):
        return best_result
    ...
    if current_ore >= obs_ore_cost and current_clay >= obs_clay_cost and time_left != 1 and robot_obs < geo_obs_cost and not c_c and not current_obs + (time_left * robot_obs) >= time_left * geo_obs_cost:
        ...
        c_c = True
    if current_ore >= clay_cost and time_left != 1 and robot_clay < obs_clay_cost and not c_b and not current_clay + (time_left * robot_clay) >= (time_left * obs_clay_cost):
...
for i in blueprint:
    result += search(i[1], i[2], i[3], i[4], i[5], i[6], minutes, 0, 0, 0, 0, 1, 0, 0, 0, True, 0, False, False, False)
    print(result)
